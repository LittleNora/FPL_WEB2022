    ...
    if (Array.isArray(data)) {
        ...
        let htmls = data.reduce((acc, product) => {
                ...
                <div class="product-item">
                    ...
                    <button product-id="${product.id}">Add To Cart</button>
                    `}
                </div>`;
            }, '');
...
function renderDetailProduct(selector) {
    ...
    if (detailElement) {
        ...
        if (window.location.search) {
            let productId = Number(new URLSearchParams(window.location.search).get('product-id'));
        ...
        <div class="product-detail-text">
            ...
            <div class="product-detail-add-cart">
                ...
                <button product-id="${dataRender.id}">Add To Cart</button>
            </div>
        </div>
...
function renderBlogPosts(selector, dataRender = ListBlogPosts, limit = 0) {
    ...
    if (blogPostsElement) {
        let blogCategoryId = Number(new URLSearchParams(window.location.search).get('blog-category'));
        if(blogCategoryId) {
...
function renderPost(selector) {
    ...
    if (postElement) {
        ...
        if (window.location.search) {
            blogId = Number(new URLSearchParams(window.location.search).get('blog-id'));
        ...
        if (cart && cart.length !== 0) {
                ...
                <tr class="cart-item cart-item-footer">
                    ...
                    <td>
                        <div class="cart-footer-control">
                            <input ${checkingCheckedAll() ? 'checked' : ''} type="checkbox" name="" id="">
            ...
            cart.reduce((acc, item) => {
                ...
                <tr class="cart-item" product-id="${item.productId}">
                    <td>
                        <input type="checkbox" ${item.isChecked ? 'checked' : ''} class="cart-item-checkbox" name="cart-checkbox">
                    </td>
                    <td>
                        <a href="./detail.html?product-id=${product.id}" class="product-info">
...
function checkSelectAllCart() {
    let selectAllBtn = document.querySelector('.cart-footer-control input[type="checkbox"]');
...
function controlQuantity(cartItemElement, cartItem, product, totalPriceElement) {
    let quantityControlElement = cartItemElement.querySelector('.quantity-control');
    ...
    inputQuantity.onchange = () => {
        if (!isNaN(Number(inputQuantity.value)) && Number(inputQuantity.value) > 0) {
